...
# returns:
    ...
    train_loader = DataLoader(dataset=train_dataset, shuffle=True,
                              ...
                              drop_last=True)
    eval_loader = DataLoader(dataset=eval_dataset, shuffle=False,
                              batch_size=4, collate_fn=collate_fn,
                              num_workers=num_workers, pin_memory=True,
                              drop_last=False)
    ...
    total_bytes = sum(
        os.path.getsize(os.path.join(tmp_dir, f)) for f in os.listdir (tmp_dir) if os.path.isfile(os.path.join(tmp_dir, f))
        )
